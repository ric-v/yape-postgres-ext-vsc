...
var require_client = __commonJS({
  "node_modules/pg/lib/client.js"(exports2, module2) {
    ...
    var crypto = require_utils2();
    var Client5 = class extends EventEmitter3 {
      constructor(config) {
        super();
  ...
  "node_modules/pg-pool/index.js"(exports2, module2) {
    ...
    var Pool = class extends EventEmitter3 {
      constructor(options, Client5) {
        ...
        this.Client = this.options.Client || Client5 || require_lib2().Client;
...
var require_client2 = __commonJS({
  "node_modules/pg/lib/native/client.js"(exports2, module2) {
    ...
    var Client5 = module2.exports = function(config) {
      EventEmitter3.call(this);
      config = config || {};
    ...
    util.inherits(Client5, EventEmitter3);
    ...
    Client5.prototype.query = function(config, values, callback) {
      var query;
      var result;
      var readTimeout;
      var readTimeoutTimer;
    ...
    Client5.prototype.setTypeParser = function(oid, format, parseFn) {
      return this._types.setTypeParser(oid, format, parseFn);
    };
    Client5.prototype.getTypeParser = function(oid, format) {
      return this._types.getTypeParser(oid, format);
    };
  }
});
...
var require_lib2 = __commonJS({
  "node_modules/pg/lib/index.js"(exports2, module2) {
    ...
    if (typeof process.env.NODE_PG_FORCE_NATIVE !== "undefined") {
      ...
      module2.exports = new PG(Client5);
...
var import_pg4 = __toESM(require_lib2());

// src/connectionForm.ts
var vscode = __toESM(require("vscode"));
var import_pg = __toESM(require_lib2());
var ConnectionFormPanel = class _ConnectionFormPanel {
  ...
  static show(extensionUri) {
    if (_ConnectionFormPanel.currentPanel) {
      _ConnectionFormPanel.currentPanel._panel.reveal();
      return;
    }
    ...
    _ConnectionFormPanel.currentPanel = new _ConnectionFormPanel(panel, extensionUri);
  }
  async _initialize() {
...
var DatabaseTreeProvider = class {
  ...
  async getChildren(element) {
    const connections = vscode2.workspace.getConfiguration().get("postgresExplorer.connections") || [];
    if (!element) {
    ...
    try {
      ...
      switch (element.type) {
        ...
        case "schema":
          return [
            new DatabaseTreeItem("Tables", vscode2.TreeItemCollapsibleState.Collapsed, "category", element.connectionId, element.databaseName, element.schema),
            new DatabaseTreeItem("Views", vscode2.TreeItemCollapsibleState.Collapsed, "category", element.connectionId, element.databaseName, element.schema),
            new DatabaseTreeItem("Functions", vscode2.TreeItemCollapsibleState.Collapsed, "category", element.connectionId, element.databaseName, element.schema)
          ];
        case "category":
      ...
      vscode2.window.showErrorMessage(`Failed to get tree items: ${err.message}`);
      return [];
    } finally {
      if (client) {
        await client.end();
...
var DatabaseTreeItem = class extends vscode2.TreeItem {
  constructor(label, collapsibleState, type, connectionId, databaseName, schema, tableName, columnName) {
    ...
    this.iconPath = {
      connection: new vscode2.ThemeIcon("server"),
      database: new vscode2.ThemeIcon("database"),
      schema: new vscode2.ThemeIcon("symbol-namespace"),
      table: new vscode2.ThemeIcon("table"),
      view: new vscode2.ThemeIcon("eye"),
      function: new vscode2.ThemeIcon("symbol-method"),
      column: new vscode2.ThemeIcon("symbol-field"),
      category: new vscode2.ThemeIcon("folder")
    }[type];
  }
};

// src/tableProperties.ts
var vscode3 = __toESM(require("vscode"));
var TablePropertiesPanel = class {
  static async show(client, schema, name, isView = false, isFunction = false) {
    try {
      if (isFunction) {
        const functionQuery = `
                    SELECT p.proname,
      ...
      const columnQuery = `
                ...
                ORDER BY a.attnum`;
      const definitionQuery = isView ? `SELECT pg_get_viewdef('${schema}.${name}'::regclass, true) as definition` : `WITH 
                columns AS (
                    SELECT string_agg(
                        format(
                            '%I %s%s%s',
      ...
      const definition = isView ? defResult.rows[0].definition : defResult.rows[0].definition.replace(/[<>]/g, (m) => m === "<" ? "&lt;" : "&gt;").replace(/\((.*)\)/s, (match, group) => {
        return "(\n  " + group.split(",").map((line) => line.trim()).join(",\n  ") + "\n)";
      });
      ...
      vscode3.window.showErrorMessage(`Error loading properties: ${err.message}`);
    }
  }
};
function formatSqlWithHighlighting(formattedScript) {
  const escapedScript = formattedScript.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  return escapedScript.replace(/\b(CREATE|TABLE|VIEW|FUNCTION|RETURNS|LANGUAGE|AS|BEGIN|END|DECLARE|IF|THEN|ELSE|RETURN|PRIMARY KEY|NOT NULL|NULL)\b/g, '<span class="keyword">$1</span>').replace(/\b(integer|text|boolean|timestamp|numeric|character varying|without time zone|bigint|smallint|real|double precision|json|jsonb|uuid|date|time|bytea)\b/g, '<span class="type">$1</span>').replace(/(\w+)\.(\w+)/g, '<span class="identifier">$1</span>.<span class="identifier">$2</span>').replace(/\b(PRIMARY KEY|UNIQUE|FOREIGN KEY|CHECK|REFERENCES)\b/g, '<span class="constraint">$1</span>').replace(/\b(plpgsql|sql|STABLE|VOLATILE|IMMUTABLE|SECURITY DEFINER|PARALLEL SAFE)\b/g, '<span class="keyword">$1</span>');
}

// src/notebookProvider.ts
var vscode4 = __toESM(require("vscode"));
var PostgresNotebookProvider = class {
  async deserializeNotebook(content, _token) {
    ...
    if (content.byteLength > 0) {
      try {
        ...
        if (Array.isArray(data.cells)) {
          cells = data.cells.map(
            (cell) => new vscode4.NotebookCellData(
              vscode4.NotebookCellKind.Code,
              cell.value,
              "sql"
            )
          );
    ...
    const notebookData = new vscode4.NotebookData(cells);
...
var import_pg3 = __toESM(require_lib2());
var PostgresKernel = class {
  constructor(messageHandler) {
    this.id = "postgres-kernel";
    this.label = "PostgreSQL Kernel";
    console.log("PostgresKernel: Initializing");
    this.controller = vscode5.notebooks.createNotebookController(
      this.id,
      "postgres-notebook",
      this.label
    );
    ...
    const types = require_lib2().types;
    ...
    try {
      ...
      if (result.command.toUpperCase().match(/^(CREATE|ALTER|DROP|TRUNCATE)/)) {
        const html = `
                ...
                `;
        const output = new vscode5.NotebookCellOutput([
          vscode5.NotebookCellOutputItem.text(html, "text/html")
        ]);
                        ...
                        <div class="output-content">
                            <div class="table-container">
                                <table id="resultTable">
                                    ...
                                    <tbody>
                                        ${rows.map(
          (row) => `<tr>${headers.map((h) => `<td>${row[h] === null ? "" : String(row[h])}</td>`).join("")}</tr>`
        ).join("")}
                                    </tbody>
                                </table>
                            </div>
        ...
        const output = new vscode5.NotebookCellOutput([
          vscode5.NotebookCellOutputItem.text(`
                        <div style="
                            padding: 10px;
                            margin: 5px 0;
                            background: var(--vscode-editor-background);
      ...
      execution.replaceOutput([
        new vscode5.NotebookCellOutput([
          vscode5.NotebookCellOutputItem.error({
            name: "Error",
...
var PostgresNotebookSerializer = class {
  async deserializeNotebook(content, _token) {
    ...
    const cells = raw.cells.map(
      (item) => new vscode6.NotebookCellData(
        item.kind === "markdown" ? vscode6.NotebookCellKind.Markup : vscode6.NotebookCellKind.Code,
        item.value,
        item.language
      )
    );
    return new vscode6.NotebookData(cells);
  }
  async serializeNotebook(data, _token) {
    const cells = data.cells.map((cell) => ({
      kind: cell.kind === vscode6.NotebookCellKind.Markup ? "markdown" : "sql",
      value: cell.value,
      language: cell.kind === vscode6.NotebookCellKind.Markup ? "markdown" : "sql"
    }));
...
function activate(context) {
  ...
  const kernel = new PostgresKernel((message) => {
    ...
    if (message.type === "custom" && message.command === "export") {
      ...
      vscode7.commands.executeCommand("postgres-explorer.exportData", {
  ...
  context.subscriptions.push(
    vscode7.commands.registerCommand("postgres-explorer.exportData", async (args) => {
      ...
      try {
        ...
        const saveUri = await vscode7.window.showSaveDialog({
          defaultUri: vscode7.Uri.file(filename),
          filters: {
            "CSV files": ["csv"],
        ...
        vscode7.window.showErrorMessage(`Export failed: ${err.message}`);
      }
    })
  );
  context.subscriptions.push(
    vscode7.commands.registerCommand("postgres-explorer.saveFile", async (args) => {
      try {
        ...
        if (saveUri) {
          ...
          vscode7.window.showInformationMessage(`Successfully exported to ${saveUri.fsPath}`);
        }
      } catch (err) {
  ...
  context.subscriptions.push(
    vscode7.workspace.registerNotebookSerializer("postgres-notebook", notebookProvider)
  );
  context.subscriptions.push(
    vscode7.workspace.registerNotebookSerializer("postgres-query", new PostgresNotebookSerializer())
  );
  context.subscriptions.push(
    vscode7.commands.registerCommand("postgres-explorer.addConnection", () => {
      ConnectionFormPanel.show(context.extensionUri);
    })
  );
  context.subscriptions.push(
    vscode7.window.registerTreeDataProvider("postgresExplorer", databaseTreeProvider)
  );
  const treeView = vscode7.window.createTreeView("postgresExplorer", {
    treeDataProvider: databaseTreeProvider,
    showCollapseAll: true
  });
  context.subscriptions.push(
    vscode7.commands.registerCommand("postgres-explorer.refreshConnections", () => {
      databaseTreeProvider.refresh();
    })
  );
  context.subscriptions.push(
    vscode7.commands.registerCommand("postgres-explorer.showTableProperties", async (item) => {
      if (!item || !item.schema || !item.connectionId) {
        vscode7.window.showErrorMessage("Invalid table selection");
        return;
      }
      const connections = vscode7.workspace.getConfiguration().get("postgresExplorer.connections") || [];
      const connection = connections.find((c) => c.id === item.connectionId);
      if (!connection) {
        vscode7.window.showErrorMessage("Connection not found");
        return;
      }
      let client;
      try {
        client = new import_pg4.Client({
          host: connection.host,
          port: connection.port,
          user: connection.username,
          password: String(connection.password),
        ...
        const errorMessage = err?.message || "Unknown error occurred";
        vscode7.window.showErrorMessage(`Failed to show table properties: ${errorMessage}`);
        if (client) {
          try {
            await client.end();
          } catch (closeErr) {
  ...
  context.subscriptions.push(
    vscode7.commands.registerCommand("postgres-explorer.showViewProperties", async (item) => {
      if (!item || !item.schema || !item.connectionId) {
        vscode7.window.showErrorMessage("Invalid view selection");
        return;
      }
      const connections = vscode7.workspace.getConfiguration().get("postgresExplorer.connections") || [];
      const connection = connections.find((c) => c.id === item.connectionId);
      ...
      try {
        ...
        const errorMessage = err?.message || "Unknown error occurred";
        vscode7.window.showErrorMessage(`Failed to show view properties: ${errorMessage}`);
        if (client) {
          try {
            await client.end();
          } catch (closeErr) {
  ...
  context.subscriptions.push(
    vscode7.commands.registerCommand("postgres-explorer.showFunctionProperties", async (item) => {
      if (!item || !item.schema || !item.connectionId) {
        vscode7.window.showErrorMessage("Invalid function selection");
        return;
      }
      const connections = vscode7.workspace.getConfiguration().get("postgresExplorer.connections") || [];
      const connection = connections.find((c) => c.id === item.connectionId);
      ...
      try {
        ...
        const errorMessage = err?.message || "Unknown error occurred";
        vscode7.window.showErrorMessage(`Failed to show function properties: ${errorMessage}`);
        if (client) {
          try {
            await client.end();
          } catch (closeErr) {
  ...
  context.subscriptions.push(
    vscode7.commands.registerCommand("postgres-explorer.connect", async () => {
      try {
        ...
        const client = new import_pg4.Client(connectionString);
        await client.connect();
        vscode7.window.showInformationMessage("Connected to PostgreSQL database");
        databaseTreeProvider.refresh();
        await client.end();
      } catch (err) {
        const errorMessage = err?.message || "Unknown error occurred";
        vscode7.window.showErrorMessage(`Failed to connect: ${errorMessage}`);
      }
    })
  );
  context.subscriptions.push(
    vscode7.commands.registerCommand("postgres-explorer.disconnect", async () => {
      databaseTreeProvider.refresh();
      vscode7.window.showInformationMessage("Disconnected from PostgreSQL database");
    })
  );
  context.subscriptions.push(
    vscode7.commands.registerCommand("postgres-explorer.refresh", () => {
      databaseTreeProvider.refresh();
    })
  );
  context.subscriptions.push(
    vscode7.commands.registerCommand("postgres-explorer.queryTable", async (item) => {
      if (!item || !item.schema) {
        return;
      }
      const query = `SELECT * FROM ${item.schema}.${item.label} LIMIT 100;`;
      const notebook = await vscode7.workspace.openNotebookDocument("postgres-notebook", new vscode7.NotebookData([
        new vscode7.NotebookCellData(vscode7.NotebookCellKind.Code, query, "sql")
      ]));
      await vscode7.window.showNotebookDocument(notebook);
    })
  );
  context.subscriptions.push(
    vscode7.commands.registerCommand("postgres-explorer.newNotebook", async (item) => {
      if (!item) {
        vscode7.window.showErrorMessage("Please select a database, schema, or table to create a notebook");
        return;
      }
      const connections = vscode7.workspace.getConfiguration().get("postgresExplorer.connections") || [];
      const connection = connections.find((c) => c.id === item.connectionId);
      ...
      const notebookData = new vscode7.NotebookData([
        new vscode7.NotebookCellData(vscode7.NotebookCellKind.Code, `-- Connected to database: ${metadata.databaseName}
-- Write your SQL query here
SELECT * FROM ${item.schema ? `${item.schema}.${item.label}` : "your_table"}
LIMIT 100;`, "sql")
      ]);
      notebookData.metadata = metadata;
      const notebook = await vscode7.workspace.openNotebookDocument("postgres-notebook", notebookData);
      await vscode7.window.showNotebookDocument(notebook);
    })
  );
  context.subscriptions.push(
    vscode7.commands.registerCommand("postgres-explorer.deleteConnection", async (item) => {
      ...
      if (answer === "Yes") {
        ...
        await config.update("postgresExplorer.connections", updatedConnections, vscode7.ConfigurationTarget.Global);
        databaseTreeProvider.refresh();
      }
    })
  );
  context.subscriptions.push(
    vscode7.commands.registerCommand("postgres-explorer.functionOperations", async (item) => {
      if (!item || !item.schema || !item.connectionId) {
        vscode7.window.showErrorMessage("Invalid function selection");
        return;
      }
      ...
      try {
        ...
        const notebookData = new vscode7.NotebookData([
          new vscode7.NotebookCellData(
            vscode7.NotebookCellKind.Markup,
...
DROP FUNCTION IF EXISTS ${item.schema}.${item.label}(${functionInfo.arguments});`,
        ...
        const errorMessage = err?.message || "Unknown error occurred";
        vscode7.window.showErrorMessage(`Failed to create function operations notebook: ${errorMessage}`);
        if (client) {
          try {
            await client.end();
          } catch (closeErr) {
  ...
  context.subscriptions.push(
    // ...existing commands...
    vscode7.commands.registerCommand("postgres-explorer.createReplaceFunction", async (item) => {
      if (!item || !item.schema || !item.connectionId) {
        vscode7.window.showErrorMessage("Invalid function selection");
        return;
      }
...
Modify the function definition below and execute the cell to update the function.`,
        ...
        const errorMessage = err?.message || "Unknown error occurred";
        vscode7.window.showErrorMessage(`Failed to create function edit notebook: ${errorMessage}`);
        if (client) {
          try {
            await client.end();
          } catch (closeErr) {
    ...
    vscode7.commands.registerCommand("postgres-explorer.callFunction", async (item) => {
      if (!item || !item.schema || !item.connectionId) {
        vscode7.window.showErrorMessage("Invalid function selection");
        return;
      }
...
SELECT ${item.schema}.${item.label}(${functionInfo.arguments ? "\n  -- Replace with actual values:\n  " + functionInfo.arguments.split(",").join(",\n  ") : ""});`,
        ...
        const errorMessage = err?.message || "Unknown error occurred";
        vscode7.window.showErrorMessage(`Failed to create function call notebook: ${errorMessage}`);
        if (client) {
          try {
            await client.end();
          } catch (closeErr) {
    ...
    vscode7.commands.registerCommand("postgres-explorer.dropFunction", async (item) => {
      if (!item || !item.schema || !item.connectionId) {
        vscode7.window.showErrorMessage("Invalid function selection");
        return;
      }
...
DROP FUNCTION IF EXISTS ${item.schema}.${item.label}(${functionInfo.arguments});`,
        ...
        const errorMessage = err?.message || "Unknown error occurred";
        vscode7.window.showErrorMessage(`Failed to create drop function notebook: ${errorMessage}`);
        if (client) {
          try {
            await client.end();
          } catch (closeErr) {
    ...
    vscode7.commands.registerCommand("postgres-explorer.editTableDefinition", async (item) => {
      if (!item || !item.schema || !item.connectionId) {
        vscode7.window.showErrorMessage("Invalid table selection");
        return;
      }
      ...
      try {
        ...
        let tableConstraints = /* @__PURE__ */ new Map();
        let columnDefs = [];
              ...
              if (!tableConstraints.has(key)) {
                ...
                switch (constraint.type) {
                  ...
                  case "FOREIGN KEY":
                    ...
                    constraintDef += `FOREIGN KEY (${constraint.columns.join(", ")}) REFERENCES ${ref.schema}.${ref.table} (${ref.columns.join(", ")})`;
                    break;
                }
...
Modify the table definition below and execute the cell to update the table structure. Note that this will create a new table - you'll need to migrate the data separately if needed.`,
        ...
        const errorMessage = err?.message || "Unknown error occurred";
        vscode7.window.showErrorMessage(`Failed to create table edit notebook: ${errorMessage}`);
        if (client) {
          try {
            await client.end();
          } catch (closeErr) {
    ...
    vscode7.commands.registerCommand("postgres-explorer.viewTableData", async (item) => {
      if (!item || !item.schema || !item.connectionId) {
        vscode7.window.showErrorMessage("Invalid table selection");
        return;
      }
...
LIMIT 100;`,
    ...
    vscode7.commands.registerCommand("postgres-explorer.dropTable", async (item) => {
      if (!item || !item.schema || !item.connectionId) {
        vscode7.window.showErrorMessage("Invalid table selection");
        return;
      }
...
DROP TABLE IF EXISTS ${item.schema}.${item.label};`,
    ...
    vscode7.commands.registerCommand("postgres-explorer.editViewDefinition", async (item) => {
      if (!item || !item.schema || !item.connectionId) {
        vscode7.window.showErrorMessage("Invalid view selection");
        return;
      }
...
Modify the view definition below and execute the cell to update the view.`,
        ...
        const errorMessage = err?.message || "Unknown error occurred";
        vscode7.window.showErrorMessage(`Failed to create view edit notebook: ${errorMessage}`);
        if (client) {
          try {
            await client.end();
          } catch (closeErr) {
    ...
    vscode7.commands.registerCommand("postgres-explorer.viewViewData", async (item) => {
      if (!item || !item.schema || !item.connectionId) {
        vscode7.window.showErrorMessage("Invalid view selection");
        return;
      }
...
LIMIT 100;`,
    ...
    vscode7.commands.registerCommand("postgres-explorer.dropView", async (item) => {
      if (!item || !item.schema || !item.connectionId) {
        vscode7.window.showErrorMessage("Invalid view selection");
        return;
      }
